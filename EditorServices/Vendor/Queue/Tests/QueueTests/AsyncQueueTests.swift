...
final class AsyncQueueTests: XCTestCase {
	...
	func testConcurrentTasksRunCanOutOfOrder() async {
		...
		await fulfillment(of: [expB, expA], timeout: 1.0, enforceOrder: true)
	}
	...
	func testTaskWaitsForPreceedingBarrier() async {
		...
		await fulfillment(of: [expA, expB], timeout: 1.0, enforceOrder: true)
	}
	...
	func testConcurrentTasksOnlyWaitForBarrier() async {
		...
		await fulfillment(of: [expA, expC, expB], timeout: 1.0, enforceOrder: true)
	}
	...
	func testAddBarrierWhileBarrierRunningBlocksSubsequenceTasks() async {
		...
		await fulfillment(of: [expA, expB, expC], timeout: 1.0, enforceOrder: true)
	}
