...
struct EditorCommandPalette: View {
  ...
  private var searchHeader: some View {
    HStack(spacing: 9) {
      ...
      if mode != .commands {
        packageSearchMenu
        Toggle(isOn: $includeIgnoredFiles) {
          Label("Include Ignored Files", systemImage: "eye.slash")
        }
        .toggleStyle(.button)
        .labelStyle(.iconOnly)
        .help("Include files excluded by .gitignore")
        Toggle(isOn: $includeBuildArtifacts) {
          Label("Search Build Artifacts", systemImage: "shippingbox")
        }
        .toggleStyle(.button)
        .labelStyle(.iconOnly)
        .help("Include build output such as .build, target, DerivedData, and dist")
        Toggle(isOn: $includeHiddenFiles) {
          Label("Search Hidden Files", systemImage: "eye")
        }
        .toggleStyle(.button)
        .labelStyle(.iconOnly)
        .help("Search dotfiles and hidden files")
        Toggle(isOn: $includeDSStore) {
          Label("Search .DS_Store", systemImage: "internaldrive")
        }
        .toggleStyle(.button)
        .labelStyle(.iconOnly)
        .help("Include .DS_Store files")
      }
  ...
  private var filteredActions: [Action] {
    ...
    return
      actions
      .filter { Self.fuzzyMatch($0.title, query: value) }
      .sorted {
        let lhs = Self.matchScore($0.title, query: value)
        let rhs = Self.matchScore($1.title, query: value)
