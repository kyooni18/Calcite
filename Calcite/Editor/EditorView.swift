  ...
  private var editorPane: some View {
    ZStack(alignment: .bottomLeading) {
      ...
      if profile.behavior.showDiagnostics {
        EditorDiagnosticOverlay(tab: tab)
...
struct CodeTextEditor: NSViewRepresentable {
  ...
  final class Coordinator: NSObject, NSTextViewDelegate {
    ...
    func configureVim(for textView: NSTextView) {
      ...
      let signature =
        ([profile.leader, String(parent.profile.behavior.tabWidth)]
      ...
      if vimController == nil {
        ...
        vimController?.engine.leader = profile.leader
        vimController?.engine.localLeader = profile.leader
